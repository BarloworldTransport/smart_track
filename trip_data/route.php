       ...
       for ($j=0; $j <= $itemCountgps-1; $j++){
       	$gps_start_latitude = $gpsxmlObject->item(0)->getElementsByTagName('Latitude')->item(0)->childNodes->item(0)->nodeValue;
             ...
             $gps_latitude = $gpsxmlObject->item($j)->getElementsByTagName('Latitude')->item(0)->childNodes->item(0)->nodeValue;
              ...
              $gps_time = $gpsxmlObject->item($j)->getElementsByTagName('Time')->item(0)->childNodes->item(0)->nodeValue;
              $gps_driver = $gpsxmlObject->item($j)->getElementsByTagName('DriverID')->item(0)->childNodes->item(0)->nodeValue;

         	$_str .= '{"latlng":['.$gps_latitude.','.$gps_longitude.'],name:"'.$gps_time.'"},';
        }
         $_str .= "]";
