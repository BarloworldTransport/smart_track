...
<body>
    ...
    $sql_fm_id = mysqli_query($con, "SELECT 
            fleet_number,
            fm_id,
            vehicle_business_unit,
            registration_number,
            fleet_group 
            FROM vehicle 
    ...
    $cred_sql = mysqli_query($con, "SELECT 
        mix_database_username,
        mix_database_password,
        mix_database_id 
        FROM mix_credentials 
        WHERE mix_database_business_unit='$vehicle_bu'");
    ...
    $api_call->GetTripsWithTotalsForVehicleInDateRange(array(
    ...
    for ($i = 0; $i <= $itemCount - 1; $i ++) {
        $gps_data[] = $xmlObject->item($i)
            ->getElementsByTagName('EndGPSID')
    ...
    $header_gps = new SoapHeader('http://www.omnibridge.com/SDKWebServices/Positioning', 'TokenHeader', array(
        'Token' => preg_replace('/\s/', '', $token)
    ), TRUE);
    $api_gps_data->__setSoapHeaders($header_gps);
    
    $api_gps_data->GetGPSPositionsInList(array(
        'SpecificGPSIDs' => array(
            'long' => $gps_data
        )
    ))
...
for ($i = 0; $i <= $itemCount - 1; $i ++) {
    ...
    $vehicle = $xmlObject->item($i)
        ->getElementsByTagName('VehicleID')
        ->item(0)->childNodes->item(0)->nodeValue;
    $driver = $xmlObject->item($i)
        ->getElementsByTagName('DriverID')
        ->item(0)->childNodes->item(0)->nodeValue;
    $trip_start = $xmlObject->item($i)
        ->getElementsByTagName('TripStart')
        ->item(0)->childNodes->item(0)->nodeValue;
    $trip_end = $xmlObject->item($i)
        ->getElementsByTagName('TripEnd')
        ->item(0)->childNodes->item(0)->nodeValue;
    if ($i <= ($itemCount - 2)) {
        $stop_time = $xmlObject->item($i + 1)
            ->getElementsByTagName('TripStart')
            ->item(0)->childNodes->item(0)->nodeValue;
    } else {
        $stop_time = "";
    ...
    $distance = $xmlObject->item($i)
        ->getElementsByTagName('TripDistance')
        ->item(0)->childNodes->item(0)->nodeValue;
    $fuel = $xmlObject->item($i)
        ->getElementsByTagName('Litres')
        ->item(0)->childNodes->item(0)->nodeValue;
    $gps_id = $xmlObject->item($i)
        ->getElementsByTagName('EndGPSID')
        ->item(0)->childNodes->item(0)->nodeValue;
    
    for ($j = 0; $j <= $itemCountgps - 1; $j ++) {
        $gps_id_list = $gpsxmlObject->item($i)
            ->getElementsByTagName('ID')
            ->item(0)->childNodes->item(0)->nodeValue;
        IF ($gps_id = $gps_id_list) {
            $gps_latitude = $gpsxmlObject->item($i)
                ->getElementsByTagName('Latitude')
                ->item(0)->childNodes->item(0)->nodeValue;
            $gps_longitude = $gpsxmlObject->item($i)
    ...
    $get_vehicle = mysqli_query($con, "SELECT fleet_number 
                                        FROM vehicle 
                                        WHERE fm_id='$vehicle'");
    $vehicle_data = mysqli_fetch_array($get_vehicle);
    $get_driver = mysqli_query($con, "SELECT driver_name 
                                        FROM driver 
                                        WHERE driver_fm_id='$driver'");
		...
		<div class="col-md-3">
			<div class="panel panel-primary" style="position: fixed;">
				<div class="panel-heading">
					<h3 class="panel-title" style="text-align: center;">
						<b>Summary</b> <br>
						<br><?php echo $s_date ?> to <?php echo $e_date ?></h3>
				</div>
			...
			<div class="panel panel-warning class" style="position: fixed;">
				...
				<div class="panel-body">
					<p align="center">
						<a
							href='route_movement.php?vehicle=<?php echo $vehicle_desc; ?>&start_date=<?php echo $start_date; ?>&end_date=<?php echo $end_date; ?>'
							onclick='OpenPopup(this.href); return false'>
							<button type="button" class="btn btn-primary btn-sm">View Trip on
								map</button>
						</a>
					</p>
					<div class="btn-group">
						<button class="btn btn-primary btn-sm dropdown-toggle"
							data-toggle="dropdown" align="center">
							<i class="fa fa-bars"></i> Export Trip Data
						</button>
						<ul class="dropdown-menu " role="menu">
							<li><a href="#"
								onClick="$('#trip_table').tableExport({type:'json',escape:'false'});">
									<img src='../images/icons/json.png' width='24px'> JSON
							</a></li>
							<li><a href="#"
								onClick="$('#trip_table').tableExport({type:'json',escape:'false',ignoreColumn:'[2,3]'});">
									<img src='../images/icons/json.png' width='24px'> JSON
									(ignoreColumn)
							</a></li>
							<li><a href="#"
								onClick="$('#trip_table').tableExport({type:'json',escape:'true'});">
									<img src='../images/icons/json.png' width='24px'> JSON (with
									Escape)
							</a></li>
							<li class="divider"></li>
							<li><a href="#"
								onClick="$('#trip_table').tableExport({type:'xml',escape:'false'});">
									<img src='../images/icons/xml.png' width='24px'> XML
							</a></li>
							<li><a href="#"
								onClick="$('#trip_table').tableExport({type:'sql'});"> <img
									src='../images/icons/sql.png' width='24px'> SQL
							</a></li>
							<li class="divider"></li>
							<li><a href="#"
								onClick="$('#trip_table').tableExport({type:'csv',escape:'false'});">
									<img src='../images/icons/csv.png' width='24px'> CSV
							</a></li>
							<li><a href="#"
								onClick="$('#trip_table').tableExport({type:'txt',escape:'false'});">
									<img src='../images/icons/txt.png' width='24px'> TXT
							</a></li>
							<li class="divider"></li>
							<li><a href="#"
								onClick="$('#trip_table').tableExport({type:'excel',escape:'false'});">
									<img src='../images/icons/xls.png' width='24px'> XLS
							</a></li>
							<li><a href="#"
								onClick="$('#trip_table').tableExport({type:'doc',escape:'false'});">
									<img src='../images/icons/word.png' width='24px'> Word
							</a></li>
							<li><a href="#"
								onClick="$('#trip_table').tableExport({type:'powerpoint',escape:'false'});">
									<img src='../images/icons/ppt.png' width='24px'> PowerPoint
							</a></li>
							<li class="divider"></li>
							<li><a href="#"
								onClick="$('#trip_table').tableExport({type:'png',escape:'false'});">
									<img src='../images/icons/png.png' width='24px'> PNG
							</a></li>
							<li><a href="#"
								onClick="$('#trip_table').tableExport({type:'pdf',pdfFontSize:'7',escape:'false'});">
									<img src='../images/icons/pdf.png' width='24px'> PDF
							</a></li>
						</ul>
					</div>
				</div>
